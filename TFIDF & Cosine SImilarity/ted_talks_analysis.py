...
tf_doc = compute_normalizedtf(descriptions)
...
idf_dict = compute_idf(descriptions)
...
def compute_query_idf(query):
    ...
    documents = descriptions  # Create a list of documents
...
# Adjusted this function to handle division by zero
def cosine_similarity(tfidf_dict_qry, df, query, doc_num):
    ...
    for keyword in tokens:  # Iterate through the words in the query
        dot_product += tfidf_dict_qry.get(keyword, 0) * df.get(keyword, [0])[doc_num]  # Compute the dot product
        qry_mod += tfidf_dict_qry.get(keyword, 0) ** 2  # Compute the query vector magnitude
        doc_mod += df.get(keyword, [0])[doc_num] ** 2  # Compute the document vector magnitude

    qry_mod = np.sqrt(qry_mod)  # Compute the query vector magnitude
    doc_mod = np.sqrt(doc_mod)  # Compute the document vector magnitude

    if qry_mod * doc_mod == 0:  # Check for zero denominator
        return 0  
    else:  # else compute the cosine similarity
...
similarity_docs = rank_similarity_docs(descriptions)
doc_names = ["Document" + str(i) for i in range(1, len(descriptions) + 1)]
print(doc_names)
